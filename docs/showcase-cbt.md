...
| O23 | **Opgelost op 2026-08-29 in `scenario-api 0.13.0`, `stubbundel-0.16.0` en `provider-0.3.0`.** Een consumer kon niet zien met wie hij praatte: de stubbundel en de provider luisterden allebei op 8090 en zeiden allebei niets over zichzelf. `GET /v1/info` geeft nu naam, versie, `bron` en de geserveerde contractversies terug. `bron` is de kern: `pipeline` als de berichten ontstaan terwijl er werkelijk iets draait, `opname` als er vastgelegd materiaal wordt afgespeeld. Ernaast in het contract en niet erin — het contract draagt wat de grens nodig heeft om te functioneren, de conventie ernaast draagt wat de bediener moet weten — additief gepubliceerd, dus squad 2 pint pas naar 0.13.0 wanneer ze willen. **Beide gates toetsen dat het antwoord waar is en niet alleen dat het endpoint reageert:** de provider mag geen opnames dragen zolang hij `pipeline` zegt, de stub moet minstens één verloop dragen om `opname` te mogen zeggen, en `serveert` wordt tegen het manifest gehouden dat de image respectievelijk de bundel zelf meedraagt. Dat is met opzet, na twee eerdere gevallen deze maand van een endpoint dat werkte terwijl het antwoord loog. Gemeten naast elkaar: de provider meldt `{"naam":"provider van showcase-CBT","versie":"0.3.0","bron":"pipeline"}`, de bundel `{"naam":"stubbundel van showcase-CBT","versie":"0.16.0","bron":"opname"}` — het verschil dat vijf dagen onzichtbaar was, nu in één regel |
