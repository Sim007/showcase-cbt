...
Deel A kent drie versies die niet door elkaar mogen lopen: de **contractversie** van een grens, de **microserviceversie** uit de pom, en de **deelsysteemversie** die pint welke microservices samen uitgaan. Die drie gaan over de showcase zelf.

Deel B — het gereedschap waarmee de showcase gedemonstreerd wordt — heeft er geen vierde bij. **Elk Deel B-artefact draagt zijn eigen `0.x.y`-reeks**, los van elk contract en los van elke deelsysteemversie:

| Artefact | Wat het is |
|---|---|
| `stubbundel-0.x.y` | de levering aan showcase-website om tegen te bouwen |
| `provider-0.x.y` | het proces dat `scenario-api` en `run-stream` echt serveert |

**Die versie beweegt met het gereedschap en niet met de inhoud die erin zit.** Welke contractversies en welke stamdata meegaan, staat in een manifest — zoals de stubbundel dat al doet. Een scenario dat verandert levert dus nieuwe inhoud en een nieuwe uitgave, maar het versienummer zegt wát voor gereedschap het is, niet wat erin zit. Dat onderscheid is dezelfde reden als bij het contract: een nummer dat twee dingen tegelijk moet zeggen, zegt geen van beide.

**En de stamdata is geen vierde soort maar Deel B-inhoud.** Ze bestaat niet in de showcase: Payment weet er niets van, de pipelines lezen haar niet, en geen enkel deelsysteem raakt haar aan. Ze bestaat zodat showcase-website weet welke stappen er zijn — een consumer aan een grens die alleen in Deel B bestaat. Wie haar bij de contracten of bij de deelsystemen zet, ordent op formaat in plaats van op eigenaarschap.
...
**Op een asynchrone grens valt de gate uiteen in twee helften.** oasdiff leest geen AsyncAPI, en het gereedschap dat dat wél doet dekt maar de helft. Dat is gemeten en niet aangenomen (`besluiten.md`, 2026-08-14):

| | Structuur van de stream | Inhoud van de berichten |
|---|---|---|
| Wat er kan breken | kanaal weg, operatie weg, berichttype weg | veld uit `required`, type wijzigt, veld verdwijnt |
| Wie het ziet | `asyncapi diff` | het register, op `JSON`-artifacts met `FORWARD` |
| Wat het níét ziet | payloads — élke schemawijziging valt in `unclassified` | kanalen en operaties |

Samen zijn dat de twee netten die een REST-grens in één gereedschap krijgt. Los is elk van beide een half net, en dat is de reden dat de eerste kandidaat alleen niet volstond.

**De compatibiliteitsrichting is een keuze per grens en geen standaard.** Welke kant compatibel moet blijven volgt uit wie schrijft en wie leest. Bij REST stuurt de consumer het verzoek en leest de provider: `BACKWARD`. Bij een stream schrijft de provider en leest de consumer: `FORWARD`. Zet je overal `BACKWARD`, dan keurt de gate op een stream juist de wijziging goed die de consumer breekt — een veld uit `required` halen. De richting staat daarom per grens in `ci/registers.env`, met de reden erbij, en een grens zonder regel laat de publicatie falen.

**En de gate meet de ontvangstvariant, niet het gepubliceerde schema.** Compatibiliteit is een uitspraak over wat de ontvanger verdraagt. Het gepubliceerde schema staat op `additionalProperties: false` — waar over wat wij versturen, en als maat voor de ontvanger zo streng dat élke additieve wijziging breaking heet. Precies de wijziging die scenario 02 als niet-brekend toont.
...
| O13 | **Half open: de payloads van `run-stream` zijn gedekt, de structuur niet.** Op een asynchrone grens valt de diff-gate uiteen in twee helften — zie de tabel in 1.9. De inhoud van de berichten staat: `ci/ontvangstschemas.sh` registreert de zes payloadschema's als `JSON`-artifacts met `FORWARD`, en die regel doet daar aantoonbaar werk. De structuur — een kanaal of operatie die verdwijnt of hernoemt — is ongedekt: de `COMPATIBILITY`-regel is voor `ASYNCAPI` een no-op, en `asyncapi diff` crasht als non-root, wat `CLAUDE.md` eist. Dat is vastgelegd als erkende schuld met een naam en een herzieningsmoment (`besluiten.md`, 2026-08-15) en niet als iets dat nog even moet. **Zolang deze helft ontbreekt gaat `run-stream` niet naar 1.0.0**, want dan beloof je onveranderlijkheid op een grens waarvan de helft niet bewaakt wordt. Publiceren van een nieuwe versie kan wel, maar alleen met een expliciete bevestiging — `CBT_ZONDER_DIFF_GATE=akkoord` — zodat het niet stilzwijgend gebeurt |
...
| O16 | **De WireMock-stream-stub spreekt de gepubliceerde spec tegen.** `run-stream 0.11.0` zegt dat de verbinding openblijft tussen runs door, dat een run over `POST /v1/runs` begint en dat er op stilte elke 20 seconden een `: hartslag` uitgaat. De stubbundel doet dat sinds `stubbundel-0.11.0`; de stub die `ci/generate-stream-stub.sh` genereert niet — die sluit na `run-afgerond`, roteert per verbinding en kent geen hartslag. Dat is vanaf nu de enige plek in deze repository die het contract tegenspreekt, en hij is van ons eigen gebruik en niet van de levering. Te bepalen: of WireMock dit überhaupt kan — een rotatie die op een POST reageert en een periodieke commentaarregel op een open verbinding zijn geen mappings maar gedrag — en zo niet, of de WireMock-stub blijft met een vastgelegde beperking of plaatsmaakt voor `ci/stubbundel/stub.js`, die het al doet. Bewust niet opgelost bij deel B: het uitzoekwerk hoort niet in die levering. Zie `besluiten.md`, 2026-08-17 |
| O17 | **Starten van scenario 00 speelt de run van 01.** De stamdata is sinds `stubbundel-0.12.0` per id gesplitst — `GET /v1/scenarios/00` levert zijn eigen 19 stappen — maar de opnames zijn dat niet: `stamdata/runs/` bevat één set en die is van scenario 01. `POST /v1/runs` kijkt niet naar het `scenarioId`, speelt de volgende opname uit de rotatie, en antwoordt met `scenarioId: "01"` ook als je 00 vraagt. Gemeten op 2026-08-21. Dezelfde klasse als de fixtures die onder een contractversie stonden, één laag lager: per id gesplitst aan de ene kant en niet aan de andere. Bewust niet half gerepareerd — de 201 het gevraagde id laten echoën maakt het geloofwaardiger en niet beter, want dan claimt het antwoord 00 terwijl de stream 27 stappen van 01 stuurt. Te doen: opnamemap per scenario, de stub laten kiezen op `scenarioId`, en een antwoord op "scenario bestaat wel, opname niet". Hangt samen met O18: als de opnames uit echte runs komen, verandert deze reparatie van vorm |
| O18 | **Er is geen draaiende showcase-CBT; de provider van `scenario-api` en `run-stream` bestaat alleen als stub.** Vastgesteld op 2026-08-21: het enige dat die twee grenzen serveert is `ci/stubbundel/stub.js`. De echte kant is een verzameling shellscripts die een rapport schrijft. Besluit van de PO is dat de website op de echte kant aansluit, en dat vraagt dus een provider: `stap()` schrijft een gebeurtenis naast de rapportregel, een klein proces volgt dat bestand en zendt SSE, en `POST /v1/runs` start het demoscript. **Niet in een container** — de pipeline doet zelf `docker build` en `docker compose up`, en dat vanuit een container vraagt de Docker-socket (root-equivalent op de host) plus padvertaling voor elke bind-mount; een hostproces geeft hetzelfde resultaat zonder die twee. Voorwaarde: `stap()` draagt de gecontroleerde vergelijking tussen scenario 00 en 01, dus er komt iets naast en die functie wordt niet verbouwd. Hieraan hangt ook of showcase-CBT daarmee een draaiend systeem wordt in plaats van een verzameling scripts — dat is O14, en dat besluit komt wanneer de provider er ligt. Zie `besluiten.md`, 2026-08-21 |
| O19 | **Een opgeslagen run per scenario is er voorlopig alleen voor het gelukkige pad.** Besluit van de PO: voor elk scenario is altijd een opgeslagen run beschikbaar, als onderdeel van "een scenario is klaar". Zodra die opnames uit échte runs komen (O18) vervalt de vraag welke stap moet falen — dat is dan een uitkomst en geen ontwerpkeuze. Maar `gestopt` en `midden` zijn niet uit een geslaagde run op te nemen: voor een gestopte run moet er werkelijk iets falen, en dat is het materiaal van scenario 3. Voor `midden` moet je op een gekozen moment aansluiten. De gestopte run is juist de sterkste beat van de demo — een deelsysteem dat nooit aan de beurt kwam — dus dit gat is niet cosmetisch. Te bepalen wanneer O18 er ligt: opnemen tijdens scenario 3, of de gestopte en middenopname gegenereerd houden naast een opgenomen gelukkig pad |
| O20 | **Het oordeel van drie gates hangt aan de woordkeus van een testgereedschap.** `testgevallen()` in `ci/lib/tools.sh` telt gedraaide testgevallen door in het uitvoerlog te grepen op `[0-9]+ passed` of `Tests run: [0-9]+`. Dat getal voedt `verwacht_minstens` in `ci/verify-contract.sh`, `ci/smoke.sh` en `ci/gebruikersflow.sh` — dus of een gate groen wordt, hangt af van hoe Schemathesis, Playwright of Maven zijn uitvoer formuleert. **Het heeft al een echte fout gemaskeerd:** bij de stille beëindiging uit O15 viel de consumerverificatie om terwijl de providerkant ontsnapte, en de reden was precies dit — zijn gereedschap gebruikte een ander woord. De uitkomst van een stap wordt níét geparsed; die komt uit de exitcode, en dat blijft zo. Dit gaat alleen over het aantal. Te bepalen: of de gereedschappen een machineleesbare uitvoer hebben (JUnit-XML, JSON-rapport) die in plaats van de grep kan komen, en zo nee, of het aantal dan uit een ander artefact moet komen dan uit tekst. Vastgesteld op 2026-08-22, herzien uiterlijk 2027-02-22 |
| O21 | **De zekering op de gebeurtenissen maakt een blinde vlek die niet rood wordt.** `stap()` stuurt elke gebeurtenis best-effort naar de provider; een onbereikbare provider kost anders een seconde per gebeurtenis — gemeten: 54 seconden bovenop een run van 131. Daarom gaat de POST na de eerste mislukking uit voor de rest van de run. **De prijs is stilte:** vanaf dat moment mist de website de rest van de run zonder dat er iets rood wordt. Dat is verdedigbaar omdat het gebeurtenissenbestand het archief is en niets verloren gaat — de run is volledig opgenomen, alleen het meekijken hapert — en omdat de PO zelf demonstreert en het op het scherm ziet. Bewust geen zichtbaarheidsmechanisme: opnieuw draaien werkt, en de opname is er. Te herzien zodra iemand anders dan de PO demonstreert, of zodra de provider ergens draait waar niemand meekijkt. Vastgesteld op 2026-08-22, herzien uiterlijk 2027-02-22 |
| O22 | **De provider is gedeeltelijk: `POST /v1/runs` ontbreekt.** `provider-0.1.0` serveert `GET /v1/scenarios`, `GET /v1/scenarios/{scenarioId}` en `GET /v1/runs/stream`, en ontvangt de gebeurtenissen van de pipeline. Starten kan hij niet: hij draait in een container en het werk gebeurt op de host, en duwen vanuit die container zou de Docker-socket vragen — root-equivalent — plus een mount van de repository. Dat is bewust niet gedaan. De oplossing is een runner die op de host zijn werk ophaalt, en die komt in de volgende versie. **Zolang die er niet is, is dit een provider die een gepubliceerde operatie niet ondersteunt**, en dat is een afwijking van het contract en geen implementatiedetail. Wie ertegen bouwt, moet weten dat de startknop van showcase-website geen antwoord krijgt. Twee dingen die bij de oplossing horen: de provider weet niet of er een runner luistert, en daar kent de spec geen status voor — dat wordt een **503** met het `Error`-schema, opnieuw een benoemde afwijking; en de **201 loopt tot een seconde voor op de werkelijkheid**, want bij ophalen begint de run binnen één pollinterval, terwijl 201 zegt dat hij gestart is. Vastgesteld op 2026-08-22, herzien uiterlijk 2026-09-22 |
| O23 | **Een consumer kan niet zien met wie hij praat.** De stubbundel en de provider luisteren allebei op 8090 en zeggen allebei niets over zichzelf. Op het scherm van showcase-website staat "verbonden met showcase-CBT" — waar, maar niet met wélke, en voor een demo is dat het verschil tussen "de pipeline draait echt" en "een opname speelt af". **In Deel A is dit al opgelost:** elke service publiceert op `/actuator/info` zijn naam, zijn deelsysteemversie en welke contractversies hij serveert, en `ci/versieconformiteit.sh` leest dat als gate. Het is dus een afgedwongen conventie en geen gewoonte — en Deel B heeft er niets van. **Oordeel: ernaast en niet in het contract.** Het contract draagt wat de grens nodig heeft om te functioneren; de conventie ernaast draagt wat de bediener moet weten. Naam en versie van een implementatie zijn een eigenschap van die implementatie, niet van de interface; in het contract gezet zou het contract voortaan zijn implementaties beschrijven in plaats van zijn grens. Het tegenargument is echt — `scenario-api` draagt de CORS-preflight, en die is óók geen domein — maar het verschil houdt: zonder de preflight wérkt de grens niet, zonder identificatie werkt hij prima en weet alleen de mens ernaast niet waar hij naar kijkt. **Voorwaarde bij het bouwen:** dezelfde conventie op stub én provider (naam, versie, geserveerde contractversies) mét een regel in `ci/toets-stubbundel.sh` en `ci/toets-provider.sh` die rood wordt als hij ontbreekt — anders is het opnieuw een belofte zonder gate, en dat is deze maand drie keer misgegaan. Het alternatief dat squad 2 nu zou moeten doen — gokken uit een poortnummer — is een aanname in het gereedschap, van dezelfde soort als de `/api/hoofdstukken`-aanname. Vastgesteld op 2026-08-22, herzien uiterlijk 2026-10-22 |
