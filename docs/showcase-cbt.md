...
| O17 | **Opgelost op 2026-08-24 in `stubbundel-0.15.0`.** De stamdata was per id gesplitst en de verlopen niet: `POST /v1/runs` keek niet naar het `scenarioId`, speelde de volgende opname uit één rotatie, en antwoordde met `scenarioId: "01"` ook als je 00 vroeg. De stub kiest nu op scenario — een start op 00 speelt `run-000000` met 19 stappen, een start op 01 speelt `run-000001` met 27 — en een onbekend scenario geeft 404 in plaats van dat er iets anders gespeeld wordt. Bij die reparatie kwam iets ergers aan het licht, gemeld door squad 2: de bundel dróég sinds 0.13.0 twee échte opnames en speelde ze nooit. Dat viel aan onze kant niet op omdat de bundeltoets tegen de provider is bedacht en niet tegen de stub. De stub weigert nu te starten als er een verloop in `runs/` staat dat het manifest niet noemt, en de toets eist dat manifest en map elkaar precies dekken |
