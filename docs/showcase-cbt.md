...
| O13 | **Op `run-stream` zijn twee halve netten nodig, en één ervan staat er.** oasdiff leest geen AsyncAPI, dus de diff-gate uit 1.9 valt op die grens weg. Onderzocht en gemeten op 2026-08-14: `asyncapi diff` classificeert wél op kanaal- en operatieniveau maar zet élke schemawijziging in `unclassified`, en de `COMPATIBILITY`-regel van het register is voor artifacttype `ASYNCAPI` een no-op — zelfs alle kanalen weghalen levert HTTP 200. Voor artifacttype `JSON` dóét die regel inhoudelijk werk, met benoemde oorzaken. Daarom worden de zes payloadschema's als losse `JSON`-artifacts afgeleid en met `FORWARD` geregistreerd (`ci/ontvangstschemas.sh`); dat net staat. Wat nog ontbreekt is de structuurkant: `asyncapi diff` als gate vóór publicatie, met de invoer gevalideerd, de uitvoer bewezen welgevormd en de parserannotaties eruit gefilterd |
